...
output "control_plane_private_ip" {
  value       = aws_spot_instance_request.k3s_control_plane.private_ip
  description = "Private IP of the K3s Control Plane Node"
}

#utput "worker_private_ip" {
...
output "kubeconfig_instructions" {
  value       = <<EOT
    ...
    ssh -i <path_to_your_key> -L 6443:${aws_spot_instance_request.k3s_control_plane.private_ip}:6443 ubuntu@${aws_instance.bastion.public_ip}
